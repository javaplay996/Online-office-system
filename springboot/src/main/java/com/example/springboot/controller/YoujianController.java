...
public class YoujianController {
    ...
    public Result delete(@PathVariable Integer id) {
        youjianService.removeById(id);
        return Result.success();
    }

    @PostMapping("/del/batch")
    public Result deleteBatch(@RequestBody List<Integer> ids) {
        youjianService.removeByIds(ids);
        return Result.success();
    }

    @GetMapping
    public Result findAll() {
        return Result.success(youjianService.list());
    }

    @GetMapping("/{id}")
    public Result findOne(@PathVariable Integer id) {
        return Result.success(youjianService.getById(id));
    }
    ...
    public Result findPage(@RequestParam(defaultValue = "") String name,
        ...
        QueryWrapper<Youjian> queryWrapper = new QueryWrapper<>();
...
//        }
        return Result.success(youjianService.page(new Page<>(pageNum, pageSize), queryWrapper));
    }
    ...
    */
    ...
    public void export(HttpServletResponse response) throws Exception {
        // 从数据库查询出所有的数据
        List<Youjian> list = youjianService.list();
        // 在内存操作，写出到浏览器
        ...
        String fileName = URLEncoder.encode("Youjian信息表", "UTF-8");
    ...
    public Result imp(MultipartFile file) throws Exception {
        ...
        List<Youjian> list = reader.readAll(Youjian.class);

        youjianService.saveBatch(list);
        return Result.success();
    }
